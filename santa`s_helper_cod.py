...
def game():
    ...
    stars = 0  # количество звезд игрока
    list_load_image = []  # список кадров гифки
    count_finish_picture = 0  # счётчик кадров последней гифки, финальной
    dict_comps_and_checks = {}  # словарь для отображения на чеке нужных компонентов и их чекбоксов
    components_in_gift = []  # то, что уже положили в подарок
    comps = []  # компоненты для подарка
    dict_excess_components = {}  # словарь для ненужных компонентов
    checkboxes = []  # это будут группы спрайтов
    components = []  
    client = []  # данные о пришедшем в магазин клиенте
    account = None  # никто не зашёл в аккаунт
    gift_image = ""  # картинка подарка
    n = 0
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if flag_main_window:
                    ...
                    if button == "старт" and account:
                        ...
                        if account is None:
                            ...
                            flag_account_window = True  # нужна проверка на вход в аккаунт,
                            # чтобы заново не выходить/входить
                ...
                if flag_assambley_window:
                    ...
                    if button == "ВЫДАТЬ":
                        ...
                        dict_excess = dict_excess_components
                ...
                if flag_shop_window:
                    ...
                    if flag_give_away:
                        if event.button == 1:
                            if image_x <= event.pos[0] <= image_x + image_width:
                                if 0 <= event.pos[1] - image_y <= image_height:
        ...
        if flag_assambley_window:
            ...
            game_level = load_level(level)  # в game_level лежит не числовое значение уровня,
            # а список приходящих клиентов
            if d:
                ...
                if comps is not None:
                    ...
                    components, checkboxes, dict_comps_and_checks, last_pos = load_receipt(comps,
                                                                                           checkbox_uncheck,
                                                                                           checkbox_check)
                    d = False
                else:
